...
environment variable so that it includes the folder where python is installed. If not, then 
you will need to manually update the PATH variable. Open the Windows Control Panel, click 
on User Accounts, and then click on Change my environment variables to edit the PATH variable.
...
Change my environment variables). Set the PYTHONPATH environment variable to the full path
of the Assembler folder. This setting allows Python to find the Assembler.py code.

For example, assume that the Python program sbasm.py is stored in the folder 
C:\Assembler_SB. This means that the Assembler itself, which is made up of the two files 
Assembler.py and ErrorCodes.py, is stored in the folder C:\Assembler_SB\Assembler.

Then, you would set PYTHONPATH as follows: PYTHONPATH = C:\Assembler_SB

Also, you need to add the same path, C:\Assembler_SB, to your PATH environment variable.
